...
TestInstance::TestInstance(string str)
{
  datasetForm = str;
...
string TestInstance::getStringRep()
{
  string s("");
...
void TestInstance::countFeats(signed char * featcounts, Individual2 * ind)
...
float TestInstance::fitnessHiFi(Individual2* individual)
...
int TestInstance::classify(Individual2* individual)
...
string TestInstance::intToBinary(int i)
...
string TestInstance::byteToString(unsigned char c)
{
  string s("");
  int mask = 128;
  for (int i=0; i < 8; i++)
...
int TestInstance::distToNearCorner(int bkr, int bkf)
...
int TestInstance::distToNearEdge(int rnk, int fl)
...
int TestInstance::lowbyte(int i)
